...
const dualTrade = async (router1, router2, baseToken, token2, amount) => {
  ...
  try {
    ...
    const txstatic = await arb.connect(owner).callStatic.dualDexTrade(router1, router2, baseToken, token2, amount, { gasLimit: 500000 }); //{ gasPrice: 1000000000003, gasLimit: 500000 }
    const tx = await arb.connect(owner).dualDexTrade(router1, router2, baseToken, token2, amount, {  gasLimit: 500000 }); //{ gasPrice: 1000000000003, gasLimit: 500000 }
